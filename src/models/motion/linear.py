...
class LinearMotionModel:
    ...
    def __call__(self, x_motion, x_last_pos, pred_length, x_last_rel, h_vel, parametrization, linear_center_only, **kwargs):
        ...
        if num_nodes > 0:
            ...
            estimate_vel[num_estimates == 0] = x_last_rel[num_estimates == 0]
            assert not torch.isnan(estimate_vel).any()

            if parametrization == 'offsets_abs_scale':
                estimate_vel[..., -1] = h_vel
